...
export const specialEqProposalSlice = createSlice({
  name: "proposal",
  initialState,
  reducers: {
...
export const {
  ...
  setError,
  setArchived,
} = specialEqProposalSlice.actions;

export const getAllProposals = () => async (dispatch) => {
  try {
    const response = await axios.get(`${END_POINT_SPECIAL_EQ}/proposal`);
...
export const getAllArchivedProposals = () => async (dispatch) => {
  try {
    const response = await axios.get(
      `${END_POINT_SPECIAL_EQ}/proposal/archived`
    );
...
export const addProposal = (data) => async (dispatch) => {
  try {
    const response = await axios.post(`${END_POINT_SPECIAL_EQ}/proposal`, data);
...
export const updateProposal = (id, data) => async (dispatch) => {
  try {
    const response = await axios.patch(
      `${END_POINT_SPECIAL_EQ}/proposal/${id}`,
      data
    );
...
export const moveProposalsToWarehouse = (data) => async (dispatch) => {
  try {
    const response = await axios.post(
      `${END_POINT_SPECIAL_EQ}/warehouse/move`,
      data
    );
    if (response.status === 200) {
      dispatch(getAllProposals());
...
export const createLocalExpense = (data, proposalId) => async (dispatch) => {
  try {
    const response = await axios.post(
      `${END_POINT_SPECIAL_EQ}/proposal/local-expense/${proposalId}`,
      data
    );
    if (response.status === 200) {
      dispatch(getAllProposals());
...
export const deleteLocalExpense = (id) => async (dispatch) => {
  try {
    const response = await axios.delete(
      `${END_POINT_SPECIAL_EQ}/proposal/local-expense/${id}`
    );
    if (response.status === 200) {
      dispatch(getAllProposals());
    }
...
export default specialEqProposalSlice.reducer;
