...
export const getAllCompanies = () => async (dispatch) => {
  try {
    const response = await axios.get(`${END_POINT}/company`);
...
export const addCompany = (data) => async (dispatch) => {
  try {
    const response = await axios.post(`${END_POINT}/company`, data);
...
export const editCompany = (data) => async (dispatch) => {
  try {
    const response = await axios.patch(`${END_POINT}/company/${data.id}`, data);
    if (response.status === 200) {
      dispatch(getAllCompanies());
      dispatch(setSuccess(true));
    }
...
export const getAllCars = () => async (dispatch) => {
  try {
    const response = await axios.get(`${END_POINT}/company-car`);
...
export const deleteCompanyCar = (id) => async (dispatch) => {
  try {
    const response = await axios.delete(`${END_POINT}/company-car/${id}`);
    if (response.status === 200) {
      dispatch(getAllCompanies());
    }
  } catch (error) {
