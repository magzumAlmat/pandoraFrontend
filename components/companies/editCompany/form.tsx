...
import {
  ...
  editCompany,
  setSuccess,
} from "@/app/store/slice/companySlice";
...
const formSchema = z.object({
  ...
  cars_number: z.array(z.object({ car_registration_number: z.string() })),
  contract: z.string(),
...
export default function ProfileForm({ company }) {
  const form = useForm<z.infer<typeof formSchema>>({
    ...
    defaultValues: {
      ...
      company_bin: company.company_bin,
      contact_name: company.contact_name,
      phone: company.phone,
      email: company.email,
      cars_number: company.company_car_numbers || [],
      contract: company.contract || "",
      contract_date:
        company.contract_date === null ? null : new Date(company.contract_date),
      contract_color: company.contract_color || "",
    },
  });

  const success = useSelector((state: any) => state.company.success);
  ...
  function onSubmit(values: z.infer<typeof formSchema>) {
    console.log(values);
    dispatch(editCompany({ ...values, id: company.id }) as any);
  }
      ...
      <form onSubmit={form.handleSubmit(onSubmit)} className=" space-y-5 ">
        ...
        <FormItem className="w-full flex flex-col">
          ...
          <Button
            ...
            onClick={(e) => {
              ...
              form.setValue(
                "cars_number",
                cars.concat({ car_registration_number: "" })
              );
        ...
        {form.watch("cars_number").map((_, index) => (
          <FormField
            key={index}
            control={form.control}
            name={`cars_number.${index}.car_registration_number`}
            render={({ field }) => (
              <FormItem className="w-full flex flex-col">
        ...
        {success && (
          <AlertComponent
            description="Компания успешно отредактирована"
            variant="success"
          />
        )}
